...
const App: React.FC<AppProp> = ({ api }) => {
  ...
  const [connState, setConnState] = useState<CONN_STATE>(
  ...
  if (connState == CONN_STATE.NOT_CONNECTED) return <NoConnection />;
  ...
  if (connState == CONN_STATE.CONNECTED) {
    ...
    if (token == "" && !doingLogin) {
      const find_token = document.cookie
        .split("; ")
    ...
    if (!got_cookie)
      return (
        <Login
          setToken={setToken}
          showLoginFailure={token != "" && !doingLogin}
        />
      );
  }
    ...
    return (
      <Container maxWidth={false} disableGutters>
        <Routes>
          <Route
            path="/Status"
            element={<Status userType={userType} api={api} />}
          />
  ...
  return (
    <Container maxWidth={false} disableGutters>
      ...
      <Routes>
        ...
        <Route
          path="/Chargers"
          element={<Chargers userType={userType} api={api} />}
        />
        ...
        <Route path="/Firmware" element={<Firmware api={api} />} />
