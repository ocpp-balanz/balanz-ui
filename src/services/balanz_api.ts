...
export default class BalanzAPI {
  ...
  set_connstate_func(setConnState: Dispatch<SetStateAction<CONN_STATE>>) {
    ...
    if (this.connected) setConnState(CONN_STATE.CONNECTED);
  }

  async reconnect(): Promise<void> {
    if (this.connected) return; // No need
    while (true) {
  ...
  connect(): void {
    ...
    if (this.setConnState) this.setConnState(CONN_STATE.NOT_CONNECTED);
    ...
    this.ws.onopen = () => {
      ...
      setTimeout(() => {
        this.connected = true;
        if (this.setConnState) this.setConnState(CONN_STATE.CONNECTED);
      }, 1500);
