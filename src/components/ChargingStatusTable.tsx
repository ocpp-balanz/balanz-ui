...
const ChargingStatusTable: React.FC<ChargingStatusTableProps> = ({
  group,
  chargerData,
  userType,
  api,
  ...
  const [connectorStates, setConnectorStates] = useState<CONNECTOR_STATES>(
    new Map(),
  );
  ...
  function offered_or_usage(group: GROUP): number {
    if (group.max_allocation != null) return group.offered;
    else return group.usage;
  }
    ...
    chargerData.map((charger) => {
      for (const [id, connector] of Object.entries(charger.connectors)) {
        ...
        result.push({
          ...
          id_tag:
            connector.transaction != undefined
              ? connector.transaction.id_tag
              : null,
          start_time:
            connector.transaction != undefined
              ? format_time(connector.transaction.start_time)
              : "",
          user_name:
            connector.transaction != undefined
              ? connector.transaction.user_name
              : null,
          usage_meter:
            connector.transaction != undefined
  ...
  const columns: GridColDef<(typeof connectorData)[number]>[] = [
    ...
    { field: "alias", headerName: "Alias", flex: 1.4 },
    {
      ...
      flex: 2.2,
      valueGetter: (value) => {
        return value == "None" ? "Unknown" : value;
      },
    },
    {
      field: "priority",
    ...
    },
    {
      ...
      renderCell: (params) => {
        if (
          userType == "Admin" &&
          ["Charging", "SuspendedEV", "SuspendedEVSE"].includes(
            params.row.status,
          ) &&
          params.row.id_tag != null
        )
          return (
            <RemoteStop
              api={api}
  ...
  const processRowUpdate = useCallback(
    async (updatedRow: GridRowModel, originalRow: GridRowModel) => {
      ...
      const payload = {
        charger_id: updatedRow.id.split("/")[0],
        connector_id: parseInt(updatedRow.id.split("/")[1]),
      };
            ...
            >
              <Stack alignItems="left" gap={0}>
                <Gauge
                  width={70}
                  height={70}
                  value={offered_or_usage(group)}
                  valueMin={0}
                  valueMax={get_max_allocation(group)}
                  startAngle={-90}
                  endAngle={90}
                  margin={{
                    left: 0,
                  ...
                  text={({ value, valueMax }) => `${value}/${valueMax} A`}
                />
              </Stack>
              {/* 
              ...
              */}
              <Stack sx={{ display: { xs: "none", md: "block" } }}>
                <Stack>
                  <Stack
                    ...
                    width="50px"
                  >
                    <ElectricalServicesIcon color="info" sx={{ scale: "1" }} />{" "}
                    {connectorData.length}
                  </Stack>
                  Total
                </Stack>
              </Stack>
              <Stack>
                <Stack direction="row" alignItems="center" gap={1}>
                  <ElectricalServicesIcon color="success" sx={{ scale: "1" }} />{" "}
                  {connectorStates.get("Available") ?? 0}
                </Stack>
                Available
              </Stack>
              <Stack>
                <Stack direction="row" alignItems="center" gap={1}>
                  <ElectricalServicesIcon color="warning" sx={{ scale: "1" }} />{" "}
                  {connectorStates.get("Charging") ?? 0}
                </Stack>
                Charging
              </Stack>
              <Stack>
                <Stack direction="row" alignItems="center" gap={1}>
                  <ElectricalServicesIcon color="primary" sx={{ scale: "1" }} />{" "}
                  {connectorStates.get("SuspendedEV") ?? 0}
                </Stack>
                SuspendedEV
              </Stack>
              <Stack>
                <Stack direction="row" alignItems="center" gap={1}>
                  <ElectricalServicesIcon
                    ...
                    sx={{ scale: "1" }}
                  />{" "}
                  {connectorStates.get("SuspendedEVSE") ?? 0}
                </Stack>
                SuspendedEVSE
              </Stack>
              <Stack sx={{ display: { xs: "none", md: "block" } }}>
                <Stack direction="row" alignItems="center" gap={1}>
                  <ElectricalServicesIcon color="success" sx={{ scale: "1" }} />{" "}
                  {connectorStates.get("Preparing") ?? 0}
                </Stack>
                Preparing
              </Stack>
              <Stack sx={{ display: { xs: "none", md: "block" } }}>
                <Stack direction="row" alignItems="center" gap={1}>
                  <ElectricalServicesIcon color="success" sx={{ scale: "1" }} />{" "}
                  {connectorStates.get("Finishing") ?? 0}
                </Stack>
                Finishing
              </Stack>
              <Stack sx={{ display: { xs: "none", md: "block" } }}>
                <Stack direction="row" alignItems="center" gap={1}>
                  <ElectricalServicesIcon color="error" sx={{ scale: "1" }} />{" "}
                  {connectorStates.get("None") ?? 0}
                </Stack>
                Unknown
              </Stack>
            </Stack>
        ...
        <AccordionDetails sx={{ m: 0, px: 0, py: 1, width: "100%" }}>
          <DataGrid
            ...
            isCellEditable={(params) =>
              ["Charging", "SuspendedEV", "SuspendedEVSE"].includes(
                params.row.status,
              ) && !["Status", "Analysis"].includes(userType)
            }
