...
const FirmwareTable: React.FC<FirmwareTableProps> = ({ api }) => {
  ...
  async function delete_firmware(firmware_id: string) {
    ...
    if (ok == 3) {
      setFirmwareData(
        firmwareData.filter((firmware) => firmware.firmware_id != firmware_id),
      );
  ...
  const columns: GridColDef<(typeof firmwareData)[number]>[] = [
    ...
    {
      field: "charge_point_vendor",
      headerName: "Vendor (regexp)",
      flex: 2,
      editable: true,
    },
    {
      field: "charge_point_model",
      headerName: "Model (regexp)",
      flex: 2,
      editable: true,
    },
    {
    ...
    {
      field: "upgrade_from_versions",
      headerName: "Upgrade from versions",
      flex: 2,
      editable: true,
    },
    {
    ...
    <Stack>
      <DataGrid
        ...
        isCellEditable={(params: GridRowModel) =>
          (params.field != "firmware_id" && params.field != "delete") ||
          (params.field == "firmware_id" &&
            params.value == BLANKFirmware.firmware_id)
        }
      />
      <Snackbar
        open={open}
