...
const FirmwareUpgradeCharger: React.FC<FirmwareUpgradeChargerProp> = ({
  api,
  charger_id,
  charger_alias,
  snack,
  ...
  const [selectedOption, setSelectedOption] = useState<FIRMWARE_OPTION | null>(
    null,
  );
  ...
  const update = async () => {
    ...
    const [ok] = await api.call("UpdateFirmware", {
  ...
  return (
    <>
      <UpgradeIcon sx={{ mt: 0.5 }} onClick={handleClickOpen}></UpgradeIcon>
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>Confirm Charger Firmware Upgrade</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Please choose firmware to install on {charger_alias} ({charger_id})
            or select cancel.
          </DialogContentText>
          <List>
