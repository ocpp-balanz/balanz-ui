...
const UserTable: React.FC<UserTableProps> = ({ api }) => {
  ...
  const processRowUpdate = React.useCallback(
    async (updatedRow: GridRowModel, originalRow: GridRowModel) => {
      ...
      for (const [key, value] of Object.entries(updatedRow)) {
        if (
          (key == "password" && value != "(hidden)") ||
          (key != "password" && value != originalRow[key])
        ) {
  ...
  const columns: GridColDef<(typeof userData)[number]>[] = [
    { field: "user_id", headerName: "ID", flex: 2, editable: true },
    {
      ...
      flex: 2,
      type: "singleSelect",
      valueOptions: ["Status", "Analysis", "SessionPriority", "Tags", "Admin"],
      editable: true,
    },
    {
      field: "description",
    ...
    <Stack>
      <DataGrid
        ...
        isCellEditable={(params: GridRowModel) =>
          (params.field != "user_id" && params.field != "delete") ||
          (params.field == "user_id" && params.value == BLANKUSER.user_id)
        }
      />
      <Snackbar
        open={open}
